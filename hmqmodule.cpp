...
inline uint256 hmqhash(const T1 pbegin, const T1 pend)
{
...
#else
    hmq_hash((unsigned char *)PyString_AsString((PyObject*) input), Py_SIZE((PyObject*) input), output);
#endif
    Py_DECREF(input);
#if PY_MAJOR_VERSION >= 3
...
static PyMethodDef HMQMethods[] = {
    { "getPoWHash", hmq_getpowhash, METH_VARARGS, "Returns the proof of work hash using hmq hash" },
    { NULL, NULL, 0, NULL }
};

#if PY_MAJOR_VERSION >= 3
static struct PyModuleDef HMQModule = {
...
PyMODINIT_FUNC PyInit_hmq_hash(void) {
    return PyModule_Create(&HMQModule);
}

#else

PyMODINIT_FUNC inithmq_hash(void) {
    (void) Py_InitModule("hmq_hash", HMQMethods);
}
#endif
